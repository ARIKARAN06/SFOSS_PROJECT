...
exports.shuffleQuestionsForTeam = shuffleQuestionsForTeam;
exports.evaluateAnswerChoice = evaluateAnswerChoice;
 ...
 */
function generateSeed(teamId, roundId, secretSalt) {
    const hmac = crypto_1.default.createHmac('sha256', secretSalt);
 ...
 */
function shuffleQuestionsForTeam(questions, teamId, roundId, secretSalt) {
    ...
    // 2. Shuffle Option Order per Question
    const optionOrder = {};
    const processedQuestions = shuffledQuestions.map((q, index) => {
        // Derive question-specific option seed
        const qSeed = masterSeed + index * 1009 + q.id.length;
        const shuffledOptions = seededShuffle(q.options, qSeed);
        optionOrder[q.id] = shuffledOptions.map((opt) => opt.id);
        return {
            ...q,
 ...
 */
function evaluateAnswerChoice(masterOptions, selectedOptionId) {
    if (!selectedOptionId) {
