...
export async function getOrCreateParticipantSession({ teamId, competitorId, roundId }: QuizSessionParams) {
  ...
  const optionOrder = session.optionOrder as Record<string, string[]>;
  ...
  for (const qId of questionOrder) {
    ...
    const optIds = optionOrder[qId] || [];
    const masterOptMap = new Map(originalQ.options.map((opt) => [opt.id, opt]));
    const orderedOpts = optIds
      .map((optId) => masterOptMap.get(optId))
